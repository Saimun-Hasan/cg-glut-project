...
{
    // Update cloud's position by translating it to the right
    tp3 = tp3 + t;
    if (tp3 > 600)
        tp3 = -300;
    glutPostRedisplay();
...
{
    // Draw a rectangle using four vertices
    glBegin(GL_QUADS);
    glVertex2d(x, y);
    glVertex2d(x, y1);
    glVertex2d(x1, y1);
...
{
    // Draw a quadrilateral using four vertices
    glBegin(GL_QUADS);
    glVertex2d(x1, y1);
    glVertex2d(x3, y3);
    glVertex2d(x4, y4);
...
void privatecar() {
    glPushMatrix();
    glTranslatef(cm + carPosX, carPosY, 0);
    makeprivatecar();
    glPopMatrix();
}
...
void handleKeypress(unsigned char key, int x, int y) {
    switch (key) {
        case 27: // Escape key
            ...
            break;
        case 32: // Space key
            // Add code here to perform some action when space key is pressed
            break;
        case 13: // Enter key
            // Add code here to perform some action when enter key is pressed
            break;
    }
}
...
void handleSpecialKeypress(int key, int x, int y) {
    switch (key) {
        case GLUT_KEY_LEFT:
            carPosX -= 20.0; // Move car left by 5 units
            break;
        case GLUT_KEY_RIGHT:
            carPosX += 20.0; // Move car right by 5 units
            break;
    }
...
int main(int argc, char** argv)
{
    ...
    // Register keyboard special keys callback function
